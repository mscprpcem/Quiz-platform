...
export default function Analytics() {
  ...
  if (loading && !data) {
    return (
      <div className="text-center py-20 text-brand-textMuted font-semibold">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in space-y-6">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white border border-brand-border p-6 rounded-xl shadow-sm gap-4">
        <div className="flex items-center space-x-4">
          <button
            onClick={() => navigate('/admin/quizzes')}
            className="p-2 border border-brand-border hover:bg-brand-bgLight text-zinc-600 rounded-lg transition-all"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            <span className="text-[10px] font-bold text-brand-textMuted uppercase tracking-widest">{data?.eventName}</span>
            <h1 className="text-2xl font-extrabold text-brand-textMain tracking-tight mt-0.5">{data?.quizTitle} Analytics</h1>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5">
          <div className="relative group">
            <button className="flex items-center space-x-1.5 bg-brand-success hover:bg-emerald-700 text-white px-3.5 py-2 rounded-lg text-xs font-bold shadow-sm transition-all cursor-pointer">
              <FileSpreadsheet size={14} />
              <span>Export Scoreboard</span>
            </button>
            <div className="absolute right-0 mt-1 bg-white border border-brand-border rounded-lg shadow-lg hidden group-hover:block w-36 z-50 text-left py-1 text-xs">
              <button onClick={() => handleExport('results', 'excel')} className="w-full text-left px-4 py-2 hover:bg-brand-bgLight text-zinc-700 font-medium">Excel (.xlsx)</button>
              <button onClick={() => handleExport('results', 'csv')} className="w-full text-left px-4 py-2 hover:bg-brand-bgLight text-zinc-700 font-medium">CSV (.csv)</button>
            </div>
          </div>
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-1 bg-white border border-brand-border rounded-lg shadow-lg hidden group-hover:block w-36 z-50 text-left py-1 text-xs">
              <button onClick={() => handleExport('responses', 'excel')} className="w-full text-left px-4 py-2 hover:bg-brand-bgLight text-zinc-700 font-medium">Excel (.xlsx)</button>
              <button onClick={() => handleExport('responses', 'csv')} className="w-full text-left px-4 py-2 hover:bg-brand-bgLight text-zinc-700 font-medium">CSV (.csv)</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Total Joined */}
        <div className="bg-white border border-brand-border p-5 rounded-xl shadow-sm flex items-center space-x-4">
          <div className="w-10 h-10 bg-brand-lightBlue text-brand-blue rounded-lg flex items-center justify-center">
            <Users size={20} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-brand-textMuted uppercase">Joined</span>
            <p className="text-xl font-bold text-brand-textMain leading-none mt-1">{data?.totalParticipants}</p>
          </div>
        </div>

        {/* Avg Response Time */}
        <div className="bg-white border border-brand-border p-5 rounded-xl shadow-sm flex items-center space-x-4">
          <div className="w-10 h-10 bg-zinc-100 text-brand-textMuted rounded-lg flex items-center justify-center">
            <Clock size={20} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-brand-textMuted uppercase">Avg Response</span>
            <p className="text-xl font-bold text-brand-textMain leading-none mt-1">{data?.averageResponseTime}s</p>
          </div>
        </div>

        {/* Highest Score */}
        <div className="bg-white border border-brand-border p-5 rounded-xl shadow-sm flex items-center space-x-4">
          <div className="w-10 h-10 bg-emerald-50 text-brand-success rounded-lg flex items-center justify-center">
            <TrendingUp size={20} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-brand-textMuted uppercase">Highest Score</span>
            <p className="text-xl font-bold text-brand-textMain leading-none mt-1">{data?.highestScore}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-brand-border p-5 rounded-xl shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-brand-textMain leading-none mt-1">{data?.completionPercentage}%</p>
          </div>
        </div>
      </div>
      ...
      {data?.totalParticipants > 0 ? (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-brand-border rounded-xl p-5 shadow-sm space-y-4">
              <h3 className="text-sm font-bold text-brand-textMain flex items-center">
                <CheckCircle size={16} className="text-brand-success mr-1.5" />
            ...
            {/* General Highlights */}
            <div className="bg-white border border-brand-border rounded-xl p-6 shadow-sm flex flex-col justify-between">
              <h3 className="text-sm font-bold text-brand-textMain border-b border-zinc-100 pb-3 flex items-center">
                <AlertTriangle className="text-brand-warning mr-1.5" size={16} />
              ...
              <button
                ...
                className="w-full text-center border border-brand-border hover:bg-brand-bgLight text-zinc-600 font-semibold py-2 rounded text-xs transition-all mt-4"
              >
                Review Question Set
              </button>
            </div>
          ...
          <div className="bg-white border border-brand-border rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-bold text-brand-textMain border-b border-zinc-100 pb-4 mb-4">Question-by-Question Diagnostics</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-zinc-200 text-sm text-left">
                <thead className="bg-brand-bgLight text-brand-textMuted uppercase text-[10px] font-bold tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-zinc-100 text-zinc-700">
                  {data?.questionAccuracy.map((q) => (
                    <tr key={q.questionId} className="hover:bg-brand-bgLight/50 transition-colors">
        ...
        <div className="bg-white border border-brand-border rounded-xl p-12 text-center text-brand-textMuted font-semibold">
